...
model.tests <- function(){
    ...
    Uni.gram.test <- readRDS("./project/test/sources/model.test.N1.gram.rds")
    ...
    N2.gram.test <- readRDS("./project/test/sources/model.test.N2.gram.rds")
    print(sprintf("N2.gram.model status: %s", checkEquals(N2.gram, N2.gram.test)))
    
    model.learn(Ngrams.count3, 'N3.gram', level='highest')
    N3.gram.test <- readRDS("./project/test/sources/model.test.N3.gram.rds")
